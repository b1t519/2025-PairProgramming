...
export function testGreedySnakeMoveBarriers(){
    // 测试数据1：蛇能够朝上移动并吃到果子
    let snake1 = [1,4,1,3,1,2,1,1];  // 蛇身坐标，依次为蛇头、第二节、第三节、蛇尾
    let fruit1 = [1,7];  // 果子坐标
    let barriers1 = [2,7,2,6,3,7,3,6,4,7,4,6,5,7,5,6,1,6,6,6,7,6,8,6];
  
    let result1 = greedySnakeMoveBarriers(snake1, fruit1, barriers1);
    assert(result1 === -1, `Test 1 Failed: Expected -1 but got ${result1}`);
